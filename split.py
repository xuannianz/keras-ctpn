...
if __name__ == '__main__':
    ...
    label_json = json.load(open('datasets/art/train_labels.json'))
    label_dir = 'datasets/art/train_labels'
    for idx, image_path in enumerate(glob.glob('datasets/art/train_images/*.jpg')):
        ...
        labels = label_json[image_fname_noext]
        cnts = []
        for label in labels:
            if label['transcription'] == '###':
                continue
            cnts.append(np.array(label['points']))
        if len(cnts) == 0:
